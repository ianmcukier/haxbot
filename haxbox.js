...
*/
geo = {"code": "br"}
var room = HBInit({ roomName: "Campeonato dos Primos", maxPlayers: 30, password:"coronga", playerName : "CAMPEONATO", public : false, geo});
room.setDefaultStadium("Classic");
room.setScoreLimit(3);
room.setTimeLimit(0.1);
...
function startSetFun(player,msg){
    ...
    room.sendChat("Série iniciada");
    room.sendChat(red+" X "+blue);
    room.sendChat("BO"+box);
...
room.onGameStart = function() {
    ...
    if(setGame){
        room.sendChat(game.set+" Jogo da Série - "+game.red.name+" "+game.red.score+" X " +game.blue.score+" "+game.blue.name)
    }
...
room.onGameTick = function() {  
    ...
    if (kickOff == false) { // simplest comparison to not charge usulessly the tick thing
        if (room.getScores().time != 0){
            ...
            gkflag = true;
            if ( spam == true ){
                room.sendChat("Goleiro do time "+game.red.name+" : " + gk[0].name + ", Goleiro do time "+game.blue.name+" : " + gk[1].name)
            }
        }
    }
    /*
...
room.onTeamGoal = function(team){ // Write on chat who scored and when.
    ...
    if (ownGoal == "") assist = playerTouchedTwice(whoTouchedBall);
     ...
     if (ownGoal != "") {
         ...
         room.sendChat("Gol!! " + whoTouchedBall[0].name +
     assist + ownGoal + " aos " +
     time + " para o time " + team_name(team));
     }
...
room.onTeamVictory = function(scores){ // Sum up all scorers since the beginning of the match.
    ...
    if(setGame){
        if(scores.blue>scores.red){
            room.sendChat("Vitória do time "+game.blue.name+"!")
            game.blue.score +=1;
            
        }else{
            room.sendChat("Vitória do time "+game.red.name+"!")
            game.red.score +=1;
        }


        if(game.red.score == 2) {
            room.sendChat("O time "+game.red.name+ "ganhou a série!");
            endSet = true;
        }
        if(game.blue.score == 2){
            room.sendChat("O time "+game.blue.name+ "ganhou a série!");
            endSet = true;
        } 


        
        if(game.set < game.box){
            room.sendChat(game.red.name+" "+game.red.score+" X "+game.blue.score+" "+game.blue.name)
            swap = true;
        }
    }
